...
### Phonic realtime model reference

This project uses the Phonic speech-to-speech realtime model as its `llm` (see
`src/agent.py`). The full `RealtimeModel`/`RealtimeSession` Python API reference
(all constructor args, methods, and source) is saved at
`docs/phonic-realtime-reference.md` — check it before tuning Phonic options
(voice, interruption/VAD, tool-calling behavior, welcome message, etc.).
...
**Project-specific exception:** this agent deliberately does NOT use handoffs, Tasks, or TaskGroups. Phonic's realtime model does not support updating tool definitions after the initial session config is sent (confirmed live: `update_tools called after config was already sent. Phonic does not support updating tools mid-session.`) — every handoff/Task/TaskGroup relies on swapping in a different tool set for its step, so with Phonic that swap silently never reaches the model, and the LLM ends up improvising with whatever tools were registered at session start. `src/agent.py` therefore keeps one persistent `SalesAgent` with its full tool set registered once, and hand-rolls sequential data collection as plain tools (`record_name`/`record_phone`/`record_email`) with prompt-driven ordering instead. Revisit this only if the Phonic plugin adds mid-session tool-update support.
